...
def handle_duplicate_execution(glue_client, rds_cursor, rds_connection, reportregion, batch_run_date, execution_status, current_time, join_start):
    ...
    try:
        ...
        dependent_jobs = [
            ...
            os.environ['dependent_job2'],
            os.environ['dependent_job3']
        ]
...
def check_bmt_report_completion(rds_cursor, reportregion, batch_run_date, rds_connection, current_time, join_start):
    ...
    """
    try:
        print("Checking BMT report completion status...")
        rds_cursor.execute(
            """SELECT COUNT(*) 
            FROM audit.sc360_reportrefreshtrigger_log 
            WHERE regionname = %s 
...
def trigger_glue_job(glue_client, rds_cursor, rds_connection, reportregion, batch_run_date, glue_job):
    ...
    try:
        ...
        glue_client.start_job_run(JobName=glue_job,Arguments={'--reportregionname': reportregion})  # Override the existing --env parameter
        d = datetime.utcnow()
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if execution_exists:
            ...
            # Check BMT report completion
            BMTFlag = check_bmt_report_completion(rds_cursor, reportregion, batch_run_date, rds_connection, current_time, join_start)
